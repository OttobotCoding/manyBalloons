...
const ACTION_LABELS: Record<ActivityAction, ActionMeta> = {
  ...
  login_blocked_unapproved: { label: 'Login blocked', color: '#e74c3c' },
...
export default function ActivityLog() {
  ...
  const { data: res, isLoading, isError } = useQuery({
    ...
    // NOTE: @tanstack/react-query v5 replaced the old `keepPreviousData: true`
    // option (used in the original JS) with this placeholderData pattern —
    // same effect (keep showing the last page's rows while the next page
    // loads), just relocated by the v5 API. The old boolean option isn't
    // part of v5's types, so this is the direct compiling equivalent.
