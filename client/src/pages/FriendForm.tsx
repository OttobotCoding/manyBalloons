/**
 * client/src/pages/FriendForm.tsx
...
import { ApiSuccess, Friend, Relationship } from '../types';

const RELATIONSHIPS: Relationship[] = ['friend', 'family', 'colleague', 'acquaintance', 'other'];
...
const EMPTY_FORM: FriendFormState = {
  name: '',
  birthday: '',
  phone: '',
  email: '',
...
type FriendFormErrors = Partial<Record<keyof FriendFormState, string>>;

// Client-side validation — mirrors server validation
function validate(form: FriendFormState): FriendFormErrors {
  const errs: FriendFormErrors = {};
...
export default function FriendForm() {
  ...
  const [form, setForm] = useState<FriendFormState>(EMPTY_FORM);
  ...
  // ── Mutations ───────────────────────────────────────────────────────────────
  const onSuccess = (res: ApiSuccess<Friend>) => {
  ...
  const { mutate: save, isPending } = useMutation({
    mutationFn: (payload: FormData) => (isEdit ? updateFriend(id as string, payload) : createFriend(payload)),
    onSuccess,
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (errors[name as keyof FriendFormState]) setErrors((prev) => ({ ...prev, [name]: '' }));
  };
